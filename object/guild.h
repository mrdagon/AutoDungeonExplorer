...
namespace SDX_ADE
{
	...
	public:
		...
		void Init()
		{
			...
			{
				探索者.emplace_back();
				探索者.back().Make(a, a, 1, "ギルメン");
				探索パーティ[0].メンバー[a] = &Guild::P->探索者[a];
			}
		...
		{
			探索者.emplace_back();
			探索者.back().Make((int)探索者.size()-1, 求人職業, 1, 求人名前);
			//控え探索者.push_back(&探索者.back());
			求人リロール();
		}
		...
		void 除名(int ID)
		{
			if (ID >= 0)
			{
				...
				//控えから除名
				アクセサリー所持数[控え探索者[-(ID + 1)]->装備[0]->ID]++;
				アクセサリー所持数[控え探索者[-(ID + 1)]->装備[1]->ID]++;
				アクセサリー所持数[控え探索者[-(ID + 1)]->装備[2]->ID]++;

				//控え探索者.erase(控え探索者.begin() -(ID+1));
			}
